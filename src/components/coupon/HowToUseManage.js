...
export const HowToUseManage = () => {
  const [user,setUser]=useState([]);
  const [flag,setFlag]=useState(true);
  useEffect( ()=>{
    (async function(){
    const {data}=await axios.get('http://localhost:3124/api/howtouse/all')
    setUser(data)
  })()
  console.log(flag);
  },[flag]);
  ...
  function del(id){
    (async function(){
    await axios.delete(`http://localhost:3124/api/howtouse/delete/${id}`);
    setFlag(!flag);
  })()
  }
  ...
  return (
    ...
    <div className='item-manage'>
      <h2 className='item-heading'>How To Use | Manage</h2>
